...
export async function getPackageData(name: string, protocol: Protocol = 'npm', timeout?: number): Promise<PackageData> {
  let error: any
  ...
  try {
    ...
    const data = protocol === 'jsr' ? await fetchJsrPackageMeta(name, timeout) : await fetchPackage(name, false, timeout)
...
export async function resolveDependency(
  ...
  const pkgData = await getPackageData(resolvedName, dep.protocol, options.timeout)
